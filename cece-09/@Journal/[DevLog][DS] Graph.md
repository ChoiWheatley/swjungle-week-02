...
모든 점의 쌍에 대해서 최단거리를 구하려면 어떻게 해야 할까? 한 점에서 다른 모든 점으로 가는 최단거리를 구하는 다익스트라를 n번 수행할 수 있다. 이 경우 복잡도는 $O(n^3)$ 이 된다. 이와 시간복잡도 자체는 동일하지만 실제적으로 더 빠르게 수행되는 알고리즘으로 플로이드-워셜 Floyd-Warshall 알고리즘이 있다.

* negative edge도 적용된다.
* negative cycle은 불가능하다.

#### 수행과정
1. cost[i][j]라는 인접행렬을 만들고, 간선의 cost또는 inf로 초기화한다.
2. 0부터 k까지의 정점만 중간intermediate으로 활용해 갈 수 있는 i부터 j까지의 최단거리를 저장하는 배열 $A^k$를 만든다.
3. 만약 $A^{k-1}[i][k]+A^{k-1}[k][j]$ 가 $A^{k-1}[i][j]$ 보다 최단거리라면, 즉 `i→j`보다 `i→k→j`가 더 최적값이면 $A^k[i][j]$를 갱신한다.
4. k가 모든 노드의 수 N이 될 때까지 반복한다.
...
for k in range(N):
  for i in range(N):
    for j in range(N):
      # if i -> k -> j is available and shortest
      if dist[i][k] + dist[k][j] < dist[i][j]:
        dist[i][j] = dist[i][k] + dist[k][j]
      # otherwise, no update
```
#### Transitive Closure
weight이 없는 유행 그래프 G의 모든 정점에 대해, 정점 i로부터 j까지 가는 경로가 있는지 결정하는 문제이다. [어떤 정점 i에서 j로 가는 직접경로는 없고, 우회경로가 있을 때 i → j로의 간선을 연결한 그래프]

* 유향그래프 $G$의 transitive closure 행렬 $A^+$란, i에서 j까지 가는 경로의 길이가 **0보다 크면** $A^+[i][j] = 1$이고, 그렇지 않으면 (0이거나 0보다 작으면) $A^+[i][j] = 0$인 행렬이다.
* 유향그래프 $G$의 reflexive transitive closure 행렬 $A^*$란, i에서 j까지 가는 경로의 길이가 **0보다 크거나 같으면** $A^*[i][j] = 1$이고, 그렇지 않으면 (0보다 작으면) $A^*[i][j] = 0$인 행렬이다.
* 위 두 행렬의 유일한 차이는, reflexive의 경우 길이가 0인 경로도 포함한다는 것이다. 그렇기 때문에, $A^*[i][i]$는 항상 1이고, $A^+[i][i]$는 i에서 i로 향하는 사이클이 존재할 때만 1이다.

##### 어떻게 연산할까?
1. 간선이 존재하는 경우 cost[i][j] = 1, 아니면 0
2. Floyd-Warshall을 돌려서 distance행렬을 얻는다.
3. 만약 distance[i][j] == inf이면, A+[i][j] 는 0이다.
4. 그렇지 않으면, A+[i][j] = 1이다.
5. A*는 A+에서 대각선diagonal만 1로 바꾸면 된다.
